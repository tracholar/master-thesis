...
\end{equation}
其中$n=|\alpha|^2$，
可以看到，Gram矩阵每一行除了对角元为1，其他元素都为$e^{-2n}$。
它是一个循环矩阵，其特征值是第一行元素对应的$M$点离散傅里叶变换系数\cite{zxd2004matrix}，
\begin{equation}
\lambda_1 = 1 + (M-1)e^{-2n}, \lambda_{2...M} = 1-e^{-2n}.
\end{equation}
所以M阶PPM最优量子检测的平均错误概率为
\begin{equation}
P_e = 1 - (\sqrt{1 + (M-1)e^{-2n}} + (M-1)\sqrt{1-e^{-2n}})^2.
\end{equation}
