...
下面以16-QAM信号为例进行描述，
接收机接收到的信号首先经过一个透过率为$T$的分束器(BS)分成两部分，一部分给零差接收机(HD)，
零差接收机通过一组POVM测量进行探测，
\begin{equation}
\hat{\Pi}_i^{HD} = \int_{D_i} dp \ket{p}\bra{p}, i=1,2,3,4.
\end{equation}
其中积分区域定义如下
\begin{equation}
\begin{split}
D_1 = \{p| p>2|r \alpha|\}, D_2 = \{p| 0 < p \le 2|r \alpha|\}, \\
D_3 = \{p| -2|r \alpha| p \le 0\}, D_4 = \{p| p \le -2|r \alpha|\}.
\end{split}
\end{equation}
其中$r=\sqrt{1-T}$是反射系数。
可以看到，不同的判定区域对应一个按照P分量分割的分区，。那么经过零差接收后，
就可以知道信号所在的区域，如图\ref{fig:QAM-Hybrid-HD-split}所示。

\begin{figure}
\centering
  \includegraphics[width=0.5\textwidth]{figures/chap3/QAM-Hybrid-HD-split}
  \caption{零差接收机对QAM信号区域划分示意图}
  \label{fig:QAM-Hybrid-HD-split}
\end{figure}

正确探测出信号所在区域的概率可以用下式计算出来
\begin{equation}
P_c^{HD}(r \alpha_{uv}) = \int_{D(\alpha)}|\bra{p}\ket{r\alpha_{uv}}|^2 dp = \frac{1}{2} \erf(\sqrt{2}p)\arrowvert_{D_L(v) \alpha}^{D_U(v) \alpha}.
\end{equation}
这里$D_L$和$D_U$与前面的定义一致。

零差接收机探测的结果前馈给后面的位移接收机，
用于位移接收机调整位移操作的策略控制。
如果零差探测得到正确的结果，那么后面就变成一个
4态区分问题，这个4态区分可以用Boundrant接收机实现。
依次假设为某个符号，并调整位移量归零该符号。
如果假设正确，那么不再发生光子计数，如果假设错误，
那么存在一定概率发生光子技术事件，
然后将假设替换成下一个符号，并重复上述步奏。
这个过程可以用下图来表示出来，在理想情况下，
下图的第四步的情况不可能出现，因为在第三步的时候，
不会再发生光子计数事件。
该4态区分的Bondurant接收机
...
最终，两个接收机联合探测的正确概率为
\begin{equation}
P_c = \frac{1}{16} \sum_{u,v} P_c^{HD}(r\alpha) P_c^{DR}(t\alpha).
\end{equation}
这里$P_c^{HD}$和$P_c^{DR}$分别是零差接收机正确探测函数和Bondurant接收机的正确探测函数。
