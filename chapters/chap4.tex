...
\end{split}
\end{equation}
其中$d_{\min} = \min_{i,j}|\bm{c}_i - \bm{c}_j|$
...
c_{m+1,j} & \le \frac{G_{j,m+1}}{c_{jj}} \\
       ...
       & \le e^{-1/2 d_{\min}^2 n} \left(  1 + 2 A_j e^{-1/2 d_{\min} n} \right)  \\
...
\begin{equation}
\begin{split}
P_e &= 1 - \Tr(\hat{X}) =  1 - \sum_x X_{ii} \\
    \le \frac{1}{N} \sum_i A_i e^{- d_{\min}^2 n}.
\end{split}
\end{equation}
即最优量子检测具有不高于$e^{- d_{\min}^2 n}$形式的渐近性能上界。
